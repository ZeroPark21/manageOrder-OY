...
- **신규 데이터**: 새로 삽입
- **변경사항 없을 경우**: 건너뛰기
- **중요**: 업데이트 시 모든 필드를 명시적으로 업데이트해야 함 (부분 업데이트 금지)

### 4.4 필드 업데이트 체크리스트
업로드 API에서 반드시 업데이트해야 하는 필드:
- [ ] content_title (콘텐츠 제목)
- [ ] creator_name (크리에이터 이름)
- [ ] gmv (총 상품 거래액)
- [ ] est_commission (예상 수수료)
- [ ] affiliate_orders (주문 수)
- [ ] shoppable_impressions (노출 수)
- [ ] like_count (좋아요 수)
- [ ] comment_count (댓글 수)
- [ ] 기타 모든 관련 필드
...
### 6.4 🚨 중요: 날짜 범위 설정
- **기본 날짜 범위**: `2025-05-01` ~ `2025-12-31`
- **실제 데이터 분포**:
  - 5월: 1개 (2025-05-23 데이터 1개만 존재)
  - 6월: 40개
  - 7월: 398개
  - 8월: 561개
  - 9월: 약 1,136개
- **정확한 전체 통계** (5월~9월):
  ...
  - 총 주문: 664개
  - 크리에이터 수: 570명
- **주의**: 5월 데이터는 1개뿐이지만, 전체 데이터 일관성을 위해 시작 날짜는 `2025-05-01`로 유지
...
- **검토 주기**: 월 1회
- **주요 변경사항**:
  - v1.1.0: 날짜 범위 설정 주의사항 추가, 필드 업데이트 체크리스트 추가
